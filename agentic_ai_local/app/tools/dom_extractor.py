...
def summarize_html(url: str, html: str, title: str | None = None) -> dict:
    ...
    def item(el):
        return {"tag": el.name, "type": el.get('type'), "name": el.get('name'), "id": el.get('id'), "placeholder": el.get('placeholder'), "aria_label": el.get('aria-label'), "text": el.get_text(' ', strip=True)[:80] or None, "css_selector": _css(el), "xpath": _xpath(el), "visible": True}
